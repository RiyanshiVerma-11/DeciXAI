...
export default function LoanWizard() {
  ...
  const { formData, updateField, saveDraft, submitApplication, step, setStep, loading } = useLoanApplication();
    ...
    <div className="min-h-screen bg-slate-100 p-6">
      <div className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white p-8 shadow-xl">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Loan Application Wizard</h1>
            <p className="text-slate-600">A production-ready multi-step loan application experience for DeciXAI.</p>
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-slate-50 p-5">
          {CurrentComponent && (
            <CurrentComponent
              data={formData[currentSection.key] || {}}
